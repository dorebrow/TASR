...
from modify_attributes import Demand_Group, modify_demands, set_exp_vals_scaled, set_path_capacities, update_selfish_edge_flows, set_so_edge_flows, modify_demands_ascaled, strip_network
from volume_operations import BPR, parameterized_BPR
from frank_wolfe_so_flows import computeAssignment
import math


def ASCALE(graph_object: Graph, commodity_object: Commodity, demand_group: Demand_Group):
    """
    This function contains the implementation for the ASCALE algorithm. It works with a single
    ...
    identical to that of SCALE() and Aloof(), but commodity demands are first scaled by 
    a term ascale_beta, then system_optimal_flows are scaled by (fraction of compliant demand / ascale_beta).
    Here, ascale_beta is chosen in the same was as in the paper in which it was first proposed
    by Bonifaci, Harks, and Schafer: ascale_beta = a + sqrt(1 - fraction of compliant demand).
    """

    number_of_compliant = 0
    fraction_of_compliant = 0

    #Get fraction of compliant agents for scaling assignment
    number_of_compliant = demand_group.unscaled_demand * demand_group.trust
    fraction_of_compliant = number_of_compliant / demand_group.unscaled_demand
    ...
    for path in ordered_paths:
        ...
        for edge in path.links:
            if edge not in exceeded_so_edges:
                actual_allowable_flow = min(actual_allowable_flow, (edge.system_optimal_flow  * (fraction_of_compliant / (1 + math.sqrt(1 - fraction_of_compliant)))) - edge.assigned_flow)
        ...
        if remaining_demand <= actual_allowable_flow:
            ...
            remaining_demand = 0
            for edge in path.links:
                if edge.assigned_flow >= (edge.system_optimal_flow * (fraction_of_compliant / (1 + math.sqrt(1 - fraction_of_compliant)))):
...
def run_sc_ascale(GRAPH: str, SINGLE_COMMODITY: tuple, TOTAL_DEMAND: str):
    """
    This function sets up the network for and runs the single-commodity version of the ASCALE algorithm, 
    ...
    network_demands = modify_demands_ascaled(sc_graph, sc_graph.commodities, TOTAL_DEMAND)
    ...
    ASCALE(sc_graph, sc_commodity, sc_demand)
    ...
    total_ASCALE_flow_assigned = 0

    for commodity in sc_graph.commodities:
        if len(commodity.paths) > 0:
            for path in commodity.paths:
                total_ASCALE_flow_assigned = total_ASCALE_flow_assigned + path.assigned_flow 
                total_br_flow_assigned = total_ASCALE_flow_assigned + path.selfish_flow

    print("Total ASCALE flow assigned: ", total_ASCALE_flow_assigned)
    print("Total selfish flow assigned: ", total_br_flow_assigned)
    print("Total System Travel Time for ASCALE: ", total_network_travel_time)
...
def run_mc_ascale(GRAPH: str, TOTAL_DEMAND: str):
    """
    This function runs the multi-commodity version of ASCALE, assuming no specific 
    ...
    #this is where we need to make sure the bottleneck capacities are correct
    ...
    network_demands = modify_demands_ascaled(constrained_graph, constrained_graph.commodities, TOTAL_DEMAND)
    ...
    ordered_demand_groups = sorted(modified_network_demands, key=lambda dg: dg.trust)
    ...
    for demand_group in ordered_demand_groups:
        demand_group.demand_serviced = 0

        if len(demand_group.commodity.paths) == 0:
            continue
        ASCALE(constrained_graph, demand_group.commodity, demand_group)

        #print(demand_group.demand_serviced)
    ...
    for edge in constrained_graph.edges:
        ASCALE_edge_latency = parameterized_BPR(edge, edge.assigned_flow)
        total_network_travel_time += (ASCALE_edge_latency * edge.assigned_flow)
