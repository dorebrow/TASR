...
#LLF: Paths recommended in order of decreasing latency (under SO assignment) first. 
...
def LLF(graph_object: Graph, commodity_object: Commodity, demand_group: Demand_Group):
    """
    This functiono contains the implementation for the Largest-Latency First (LLF) algorithm.
...
'''This function sets up the network for and runs the single-commodity version of the LLF algorithm, assuming a single commodity
has been passed to the script.'''
def run_sc_llf(GRAPH: str, SINGLE_COMMODITY: tuple, TOTAL_DEMAND: str):
    ...
    LLF(sc_graph, sc_commodity, sc_demand)
    ...
    total_llf_flow_assigned = 0

    for commodity in sc_graph.commodities:
        if len(commodity.paths) > 0:
            for path in commodity.paths:
                total_llf_flow_assigned = total_llf_flow_assigned + path.assigned_flow 
                total_br_flow_assigned = total_llf_flow_assigned + path.selfish_flow

    print("Total LLF flow assigned: ", total_llf_flow_assigned)
    print("Total selfish flow assigned: ", total_br_flow_assigned)
    print("Total System Travel Time for LLF: ", total_network_travel_time)
...
''''This function runs the multi-commodity version of LLF, assuming no specific commodity has been passed to the script.'''
def run_mc_llf(GRAPH: str, TOTAL_DEMAND: str):
    graph_net_file = str(GRAPH) + "_net.tntp"
    graph_trips_file = str(GRAPH) + "_trips.tntp"
    ...
    for demand_group in ordered_demand_groups:
        if len(demand_group.commodity.paths) == 0:
            continue
        LLF(constrained_graph, demand_group.commodity, demand_group)
    ...
    for edge in constrained_graph.edges:
        llf_edge_latency = parameterized_BPR(edge, edge.assigned_flow)
        total_network_travel_time += (llf_edge_latency * edge.assigned_flow)
