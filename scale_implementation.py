...
from modify_attributes import Demand_Group, modify_demands, set_exp_vals, set_path_capacities, update_selfish_edge_flows, set_so_edge_flows
from volume_operations import BPR, parameterized_BPR
from frank_wolfe_so_flows import computeAssignment


def SCALE(graph_object: Graph, commodity_object: Commodity, demand_group: Demand_Group):
    """
    ...
    number_of_compliant = demand_group.demand * demand_group.trust
    fraction_of_compliant = number_of_compliant / demand_group.demand
    ...
    for path in ordered_paths:
        ...
        for edge in path.links:
            if edge not in exceeded_so_edges:
                actual_allowable_flow = min(actual_allowable_flow, (fraction_of_compliant * edge.system_optimal_flow) - edge.assigned_flow)
        ...
        if remaining_demand <= actual_allowable_flow:
            ...
            remaining_demand = 0
            for edge in path.links:
                if edge.assigned_flow >= (fraction_of_compliant * edge.system_optimal_flow):
...
'''This function sets up the network for and runs the single-commodity version of the SCALE algorithm, assuming a single commodity
has been passed to the script.'''
def run_sc_scale(GRAPH: str, SINGLE_COMMODITY: tuple, TOTAL_DEMAND: str):
    ...
    SCALE(sc_graph, sc_commodity, sc_demand)
    ...
    for path in sc_commodity.paths:
        for edge in path.links:
            edge.volume = path.assigned_flow
            BPR(edge)
            total_network_travel_time += edge.travel_time * edge.volume

    total_scale_flow_assigned = 0

    for commodity in sc_graph.commodities:
        if len(commodity.paths) > 0:
            for path in commodity.paths:
                total_scale_flow_assigned = total_scale_flow_assigned + path.assigned_flow 
                total_br_flow_assigned = total_scale_flow_assigned + path.selfish_flow

    print("Total SCALE flow assigned: ", total_scale_flow_assigned)
    print("Total selfish flow assigned: ", total_br_flow_assigned)
    print("Total System Travel Time for SCALE: ", total_network_travel_time)
...
''''This function runs the multi-commodity version of SCALE, assuming no specific commodity has been passed to the script.'''
def run_mc_scale(GRAPH: str, TOTAL_DEMAND: str):
    graph_net_file = str(GRAPH) + "_net.tntp"
    graph_trips_file = str(GRAPH) + "_trips.tntp"
    ...
    for demand_group in ordered_demand_groups:
        if len(demand_group.commodity.paths) == 0:
            continue
        SCALE(constrained_graph, demand_group.commodity, demand_group)
    ...
    for edge in constrained_graph.edges:
        scale_edge_latency = parameterized_BPR(edge, edge.assigned_flow)
        total_network_travel_time += (scale_edge_latency * edge.assigned_flow)
